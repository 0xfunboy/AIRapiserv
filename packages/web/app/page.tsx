  ...
  return (
    <section className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-400">Lag, providers e storage layer health</p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard title="Redis" value={status.redis} subtitle="Cache realtime" />
        <StatCard title="ClickHouse" value={status.clickhouse} subtitle="Time series" />
        <StatCard title="Postgres" value={status.postgres} subtitle="Catalogo" />
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <h3 className="text-lg font-semibold mb-2">Event rate (demo)</h3>
        <p className="text-sm text-slate-400">Quando gli ingest worker sono attivi questo box mostra il ritmo trades/tickers per market.</p>
