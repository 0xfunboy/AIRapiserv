  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400">Rolling candles multi source (demo data)</p>
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <p className="text-sm text-slate-400">
          In produzione verranno usati lightweight-charts per plot dinamici. Qui mostriamo un estratto numerico per mantenere la pagina SSR friendly.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-2 text-sm">
          {candles.slice(0, 10).map((candle: any) => (
            <div key={candle.startTs} className="border border-slate-800 rounded-lg p-3">
              ...
              <p className="text-slate-400">O: {candle.open} H: {candle.high} L: {candle.low} C: {candle.close}</p>
            </div>
          ))}
        </div>
      </div>
